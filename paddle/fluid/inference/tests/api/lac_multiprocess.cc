...
void TestLACPrediction(const std::string &model_path,
  ...
  if (use_analysis) {
    ...
    CHECK(fuse_statis.count("fc_fuse"));
    CHECK(fuse_statis.count("fc_gru_fuse"));
    CHECK_EQ(fuse_statis.at("fc_fuse"), 1);
    CHECK_EQ(fuse_statis.at("fc_gru_fuse"), 4);
    CHECK_EQ(num_ops, 11);
  }
}
...
int main(int argc, char **argv) {
  ...
  std::vector<int> pids(FLAGS_num_processes);

  for (int i = 0; i < FLAGS_num_processes; i++) {
    if ((pids[i] = fork()) < 0) {
      perror("fork");
      abort();
    } else if (pids[i] == 0) {
