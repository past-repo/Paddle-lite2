...
void TestLACPrediction(const std::string &model_path,
  ...
  ana_config.model_dir = model_path;
  ana_config.use_gpu = false;
  ana_config.device = 0;
  ana_config.specify_input_name = true;
  ana_config.enable_ir_optim = true;
  ...
  if (use_analysis) {
    ...
    LOG(INFO) << fuse_statis.at("fc_fuse");
    LOG(INFO) << num_ops;
  }
}
